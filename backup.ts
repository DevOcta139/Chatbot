...
export async function POST(req: NextRequest): Promise<NextResponse<AssistantResponse>> {
  ...
  try {
    ...
    if (currentPhase === 'general') {
      ...
      // Handle general questions with AI and check for mentioned products
      const { reply: aiResponse, mentionedProducts } = await handleGeneralQuestion(query, history, mappedProducts);

      let finalReply = aiResponse;

      // If specific products were mentioned, add suggestion to view them
      if (mentionedProducts && mentionedProducts.length > 0) {
        // Use the customer's original query instead of specific product names
      ...
      return NextResponse.json({
        reply: finalReply,
        history: manageConversationHistory(history, query, finalReply),
        phase: 'general'
      });
    }
    ...
    if (isVagueProductRequest(query)) {
      // Instead of showing random products, ask for specification
...
`;
    ...
    if (recommendationKeywords && productKeywords.length === 0) {
      const randomQuestion = config.vagueProductQuestions[
        Math.floor(Math.random() * config.vagueProductQuestions.length)
      ];
